...
export function useImageUpload(options: UseImageUploadOptions = {}) {
  ...
  const checkFileSize = useCallback((file: File): boolean => {
    if (file.size > maxFileSize) {
      onError(`파일 크기는 ${maxFileSize / (1024 * 1024)}MB 이하여야 합니다.`);
      return false;
    }
    return true;
  }, [maxFileSize, onError]);
  ...
  // 단일 이미지 핸들러
  const handleSingleImageChange = useCallback(async (file: File | null) => {
    if (file) {
  ...
  // 다중 이미지 핸들러
  const handleMultiImageChange = useCallback(async (file: File | null, index: number) => {
    // 파일이 없는 경우 (삭제 상황)
    if (!file) {
